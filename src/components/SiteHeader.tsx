...
interface SiteHeaderProps {
  /** Tailwind text color class for the logo (e.g. "text-blackout", "text-move-leaf") */
  logoColor?: string;
  /** Called when the desktop CONTACTO button or mobile contact menu item is clicked. Falls back to mailto. */
  onContactClick?: () => void;
}

export default function SiteHeader({ logoColor = "text-blackout", onContactClick }: SiteHeaderProps) {
  return (
    <nav className="fixed top-0 left-0 w-full z-[9999] bg-[#F8F6F4] border-b border-blackout/5 px-8">
      <div className="flex justify-between items-center py-4">
        <a href="/" className="hover:opacity-60 transition-opacity">
          <LunesLogo className={`h-6 md:h-7 w-auto ${logoColor}`} />
        </a>
        ...
        {onContactClick ? (
          <button onClick={onContactClick} className="hidden md:block border border-blackout/20 rounded-full px-8 py-2.5 text-[10px] uppercase tracking-[0.5em] font-sans font-bold text-blackout/60 hover:bg-blackout hover:text-white transition-all duration-300">
            CONTACTO
          </button>
        ) : (
